...
for skew in 0 1.05 1.25 ; do
	...
	for logGroupSize in ${logGroupSizes[@]} ; do
		...
		make clean -C ../aggregate_simdh_mult/
		#echo $logGroupSize
		#echo $groupSize
		make logGroups=${logGroupSize} -C ../aggregate_simdh_mult/	

		../aggregate_simdh_mult/aggregate /slowdisk/$size/$skew/data_${groupSize}.tbl $thread $groupSize $numOfTuples &>> $1/raw_results_${size}_${skew}.txt
